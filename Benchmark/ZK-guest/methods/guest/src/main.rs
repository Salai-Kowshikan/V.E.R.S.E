...
// // ----- Fixed-point helpers -----
// const SCALE: i64 = 1_000_000; // 6 decimal precision
...
// fn linear_regression_int(x: f32, a: f32, b: f32) -> f32 {
//     from_fx(mul_fx(to_fx(x), to_fx(a)) + to_fx(b))
// }
// fn multiple_regression_int(xs: &[f32], weights: &[f32], b: f32) -> f32 {
//     let weights_fx: Vec<i64> = weights.iter().map(|w| to_fx(*w)).collect();
//     let b_fx = to_fx(b);
//     let mut acc: i128 = 0;
//     for (x, w_fx) in xs.iter().zip(weights_fx.iter()) {
//         acc += (to_fx(*x) as i128 * *w_fx as i128) / SCALE as i128;
//     }
//     from_fx(acc as i64 + b_fx)
// }
// fn linear_regression(x: f32, a: f32, b: f32) -> f32 { x * a + b }
// fn multiple_regression(xs: &[f32], weights: &[f32], b: f32) -> f32 {
//     xs.iter().zip(weights.iter()).map(|(x, w)| x * w).sum::<f32>() + b
// }
// fn polynomial_regression(x: f32, coeffs: &[f32]) -> f32 {
//     coeffs.iter().enumerate().map(|(i, a)| a * x.powi(i as i32)).sum()
// }
// fn logistic_regression(xs: &[f32], weights: &[f32], b: f32) -> f32 {
...
// // ----- Timing Utility (cycle approximation) -----
// fn guest_timer() -> u64 {
//     // Not real time: deterministic tick counter surrogate
//     static mut COUNTER: u64 = 0;
//     unsafe {
//         COUNTER = COUNTER.wrapping_add(1_000);
//         COUNTER
//     }
// }
...
//             .iter()
//             .map(|(features, y_true)| {
//                 let y_pred = if use_opt {
//                     linear_regression_int(features[0], weights[0], b)
//                 } else {
//                     linear_regression(features[0], weights[0], b)
//                 };
//                 (y_pred, *y_true)
//             })
//             .collect(),
...
//             .map(|(features, y_true)| {
//                 let y_pred = if use_opt {
//                     multiple_regression_int(features, &weights, b)
//                 } else {
//                     multiple_regression(features, &weights, b)
//                 };
//                 (y_pred, *y_true)
//             })
//             .collect(),
//         3 => dataset
//             .iter()
//             .map(|(features, y_true)| (polynomial_regression(features[0], &weights), *y_true))
//             .collect(),
//         4 => dataset
//             .iter()
//             .map(|(features, y_true)| (logistic_regression(features, &weights, b), *y_true))
//             .collect(),
//         _ => panic!("Unknown model type {}", model_type),
//     };
...
//     // ----- Performance Summary -----
//     let dataset_gen_cycles = t1 - t0;
//     let compute_cycles = t2 - t1;
//     let total_cycles = t2 - t0;
...
//     // Commit results + performance data
//     env::commit(&(results, perf_stats));
// }
